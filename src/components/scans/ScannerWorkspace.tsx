                ...
                <button key={scan.id} type="button" onClick={() => router.push(`/scans/${scan.id}`)} className="flex w-full items-start justify-between gap-3 p-3.5 text-left transition hover:bg-slate-50">
                  <div className="min-w-0 flex-1">
                    <div className="flex items-center gap-2">
                      <span className={`grid h-7 w-7 shrink-0 place-items-center rounded-[6px] ring-1 ${scannerTone(scan.scanner)}`}><Icon className="h-3.5 w-3.5" /></span>
                      <p className="truncate text-[12px] font-black text-slate-950">{scan.scanner_name}</p>
                      <Badge className={statusClass(scan.status)}>{titleCase(scan.status)}</Badge>
                    </div>
                    <p className="mt-1 truncate text-[11px] font-bold text-slate-700">{scan.asset_name}</p>
                    <p className="truncate font-mono text-[9px] text-slate-400">{scan.target}</p>
                    <div className="mt-2 flex items-center justify-between text-[9px] font-semibold text-slate-500">
                      <span>{titleCase(scan.stage)} ({scan.progress}%)</span>
                      <span>{formatDate(scan.created_at)}</span>
                    </div>
                    <div className="mt-1 h-1.5 overflow-hidden rounded-full bg-slate-100">
                      <div className="h-full rounded-full bg-[#2ECE82] transition-all" style={{ width: `${scan.progress}%` }} />
                    </div>
                  </div>
